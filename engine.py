...
async def process_push(project_id: str, before: str, after: str):
    ...
    # 1. Get the diff
    try:
        diff_text = await gh.get_push_diff(owner, repo, before, after)
    ...
    # 3. Get existing docs
    doc_files = await gh.get_all_doc_files(owner, repo, docs_path)
    ...
    try:
        result = await _create_doc_pr(owner, repo, default_branch, doc_updates, project_id)
...
async def scan_repo(project_id: str):
    ...
    # Get full file tree
    tree = await gh.get_tree(owner, repo, project["default_branch"])
    source_files = [
        f for f in tree
        if f["type"] == "blob"
...
async def _create_doc_pr(owner: str, repo: str, base_branch: str,
                          doc_updates: list[dict], project_id: str) -> dict:
    """Create a branch, commit all doc updates, open a PR."""
    timestamp = datetime.utcnow().strftime("%Y%m%d-%H%M%S")
    branch_name = f"{Config.TOME_BRANCH_PREFIX}docs-update-{timestamp}"

    # Get base branch SHA
    base_sha = await gh.get_default_branch_sha(owner, repo, base_branch)

    # Create branch
    created = await gh.create_branch(owner, repo, branch_name, base_sha)
    if not created:
    ...
    for update in doc_updates:
        ...
        existing_sha = await gh.get_file_sha(owner, repo, path, branch_name)
        ...
        await gh.create_or_update_file(
            owner, repo, path, update["content"], msg, branch_name, existing_sha
        )
...
---
*Generated automatically by [Tome](https://tome.sh) — your autonomous docs engineer.*
"""

    title = f"docs: update documentation ({len(doc_updates)} {'file' if len(doc_updates) == 1 else 'files'})"

    pr = await gh.create_pull_request(owner, repo, title, pr_body, branch_name, base_branch)
